...
class Error(commands.Cog):
    ...
    async def on_slash_command_error(self,inter, exception):
        embed = discord.Embed(description=f"<a:load:1256975206455447643> Произошла ошибка при выполнении команды, выясняю причину..", color=0x2f3136)
        ...
        if isinstance(exception, commands.MissingRole):
            ...
            responce_embed = discord.Embed(description=f'## Неизвестная ошибка при выполнении команды /{inter.data.name} \nОтдел разработки уже получил уведомление о неправильной работе команды и в ближайшее время восстановит работу команды. \n\nПопробуйте использовать команду в любом другом канале Discord сервера.', color=0xE36565)
            ...
            await inter.response.edit_original_response(embed = system_embed)

            system_embed = discord.Embed(description=f'## Неизвестная ошибка при выполнении команды /{inter.data.name} \nСообщение ошибки: {traceback.format_exception(exception)}', color=0xE36565)
