...
class PlayerCMD(commands.Cog):
    ...
    async def give_money(self, inter, card_id: str, sum: int, comment: str):
        ...
        if owner_balance < sum:
            await inter.send(
                f'{config.deny} На карте `FW-{owner_card_id}` недостаточно средств (Баланс: `{owner_balance}` алмазов, а снимается `{sum}` алмазов).',
                ephemeral=True)
            return
        owner_balance -= sum
        ...
        responce_owner_pm = discord.Embed(
            ...
            color=0xEFAF6F)
        responce_owner_pm.set_footer(text=f'{main.copyright()}',
                                     icon_url=f'https://cdn.discordapp.com/attachments/1053188377651970098/1238899111948976189/9.png?ex=6640f635&is=663fa4b5&hm=541eea40573fd92a3861ed259706dff887d9934650b5aab7f698c0e9842cf9bd&')
    ...
    async def pay_invoice(self, inter, invoice_id=str):
        ...
        if type == 'Штраф':
            ...
            msg = await webhook.get_notify(msg_id)
            ...
            await webhook.edit_notify(msg_id, msg_embed)
    ...
    async def balance(self, inter, member: discord.Member = None):
        ...
        if member != None:
            if banker_role not in inter.author.roles:
                ...
                responce.set_footer(text=f'{main.copyright()}',
                                    icon_url=f'https://cdn.discordapp.com/attachments/1053188377651970098/1238899111948976189/9.png?ex=6640f635&is=663fa4b5&hm=541eea40573fd92a3861ed259706dff887d9934650b5aab7f698c0e9842cf9bd&')
            else:
        ...
        for x in card_info:
            ...
            responce.add_field(inline=False, name=f'Карта `FW-{card_id}`',
                               value=f"Баланс: `{card_balance}`. \nОформлена банкиром {banker.mention}. \nДата оформления: `{card_opendate}`")
